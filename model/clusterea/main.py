...
# 3731002
def step2_batch_similarity(ea, embeddings):
    if skip_if_complete:
        ok = True
        try:
    ...
    return (framework, metis_gcn_partition_matrix, metis_gcn_partition_matrix_t, kmeans_xgb_partition_matrix)
...
@torch.no_grad()
def step3_fuse_and_csls(embeddings, framework, metis_gcn_partition_matrix, metis_gcn_partition_matrix_t, kmeans_xgb_partition_matrix):
  ...
  #  framework, metis_gcn_partition_matrix, metis_gcn_partition_matrix_t, kmeans_xgb_partition_matrix = \
   #     load_curr_objs(PHASE_PARTITION)
    ...
    write_tensor_to_file(final_csls, f'final_csls.csv')
    write_tensor_to_file(embeddings, f'embeddings.csv')
    return acc, csls_time, fuse_time, final_csls
...
def write_tensor_to_file(tensor, file_path):
    ...
    file_path (str): The path to the file where the tensor will be written.
    """
    with open(file_path, 'w') as f:
        # Convert the tensor to a nested list for easy readability
...
def run_1_to_3(dataset):
    ...
    ea = dataset
    print('------------Training------------------')
    ea, embeddings = step1_training(ea)
    print('------------Get Sim Matrix------------------')
    (framework, metis_gcn_partition_matrix, metis_gcn_partition_matrix_t, kmeans_xgb_partition_matrix) = step2_batch_similarity(ea, embeddings)
    print('------------Fuse Sim Matrix------------------')
    acc, t1, t2, csls = step3_fuse_and_csls(embeddings, framework, metis_gcn_partition_matrix, metis_gcn_partition_matrix_t, kmeans_xgb_partition_matrix)
    add_log('acc', acc)
...
def run():
    ...
    if step == 1:
        print('------------Training------------------')
        ea, embeddings = step1_training()
    elif step == 2:
        print('------------Get Sim Matrix------------------')
        (framework, metis_gcn_partition_matrix, metis_gcn_partition_matrix_t, kmeans_xgb_partition_matrix) = step2_batch_similarity(ea, embeddings)
    elif step == 3:
        print('------------Fuse Sim Matrix------------------')
        acc, t1, t2 = step3_fuse_and_csls(embeddings, framework, metis_gcn_partition_matrix, metis_gcn_partition_matrix_t, kmeans_xgb_partition_matrix)
        add_log('acc', acc)
        add_log('global_time', t1)
        add_log('fuse_time', t2)
